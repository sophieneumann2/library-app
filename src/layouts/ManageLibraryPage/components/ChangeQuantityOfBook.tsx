...
export const ChangeQuantityOfBook: React.FC<{ book: BookModel, deleteBook: any }> = (props, key) => {
    ...
    async function increaseQuantity() {
        const url = `${API_ROUTES.BASE}/admin/secure/increase/book/quantity?bookId=${props.book.id}`;
    ...
    async function decreaseQuantity() {
        const url = `${API_ROUTES.BASE}/admin/secure/decrease/book/quantity?bookId=${props.book.id}`;
        const requestOptions = {
            method: "PUT",
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    async function deleteBook() {
        const url = `${API_ROUTES.BASE}/admin/secure/delete/book?bookId=${props.book.id}`;
        const requestOptions = {
            method: "DELETE",
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    return (
        <div className={"card mt-3 shadow p-3 p-3 mb-3 bg-body rounded"}>
            <div className={"row g-0"}>
                <div className={"col-md-2"}>
                    <div className={"d-none d-lg-block"}>
                        {props.book.img ?
                            <img src={props.book.img} width={123} height={126} alt={"Book"}/> :
                ...
                <button className={"m1 mb-1 mt-3 btn btn-md main-color text-white"} onClick={increaseQuantity}>Add
                    Quantity
                </button>
